...
class SAcctMgr(object):
    def __init__(self):
        ...
        self.bin = self.module.get_bin_path(
            "sacctmgr", True, ["/opt/slurm/bin", "/cm/shared/apps/slurm/current/bin"]
        )
    ...
    def list(self):
        ...
        cmd = (
            ["-nP", "list", self.entity, "format=" + ",".join(self.format)]
            + self.args
            + self.filter
        )
