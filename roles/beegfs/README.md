...
- `beegfs_oss`: Dict of object storage server configurations, keyed by port. Each entry:
  ...
  - `interfaces`: Optional. Overrides `beegfs_interfaces` for this OSS. If omitted and `numa_zone` is set, this is `beegfs_interfaces` reordered so the NIC(s) local to `numa_zone` come first - NUMA-based reordering only works when `beegfs_interfaces` entries are plain interface names, since it looks each one up under `/sys/class/net/`.
  - `interfaces_addr` / `interfaces_protocol`: Optional. Override `beegfs_conn_interfaces_addr` / `beegfs_conn_interfaces_protocol` for this OSS's own `connInterfacesFile`.
